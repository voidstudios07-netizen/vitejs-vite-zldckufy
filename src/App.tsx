...
function Dashboard() {
  ...
  return <section><Header eyebrow="Analytics" title="CRM dashboard" subtitle="Live outbound velocity, deal value, maintenance revenue, and weighted forecast." /><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4"><Metric label={f.ownerVisible ? 'Deal Value' : 'Your Deal Value'} value={money(f.rawPipeline)} bar="bg-cyan-300" /><Metric label={f.ownerVisible ? 'Monthly Maintenance' : 'Your Monthly Maintenance'} value={money(f.monthlyMaintenance)} bar="bg-emerald-300" /><Metric label="Expected Revenue" value={f.ownerVisible ? money(f.expectedRevenue) : 'Owner-only'} bar="bg-violet-300" /><Metric label="Retainer MRR" value={f.ownerVisible ? money(f.mrr) : 'Owner-only'} bar="bg-amber-300" /></div>
