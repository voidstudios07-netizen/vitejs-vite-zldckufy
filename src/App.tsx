...
function GlobalSearch() {
  ...
  const [results, setResults] = useState<{ leads: Lead[]; accounts: Account[]; contacts: Contact[] } | null>(null);
  const [searching, setSearching] = useState(false);
  useEffect(() => {
    if (q.trim().length < 2) { setResults(null); return; }
    setSearching(true);
    const t = setTimeout(() => {
      apiFetch<any>(`/api/search?q=${encodeURIComponent(q.trim())}`).then(setResults).catch(() => setResults(null)).finally(() => setSearching(false));
    }, 300);
    return () => clearTimeout(t);
  }, [q]);
  const totalResults = (results?.leads.length || 0) + (results?.accounts.length || 0) + (results?.contacts.length || 0);
  return <>
    <button onClick={() => setOpen(true)} className="grid h-9 w-9 place-items-center rounded-lg border border-white/10 text-slate-300 hover:text-white"><Search size={16} /></button>
    {open && <div className="fixed inset-0 z-50 grid place-items-start justify-center bg-black/70 p-4 pt-[10vh]" onClick={() => { setOpen(false); setQ(''); setResults(null); }}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-lg max-h-[70vh] overflow-y-auto rounded-3xl border border-white/10 bg-[#101218] p-4 shadow-2xl">
        <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[.03] px-3 py-2.5"><Search size={16} className="text-slate-500" /><input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search leads, accounts, contacts..." className="w-full bg-transparent text-sm outline-none placeholder:text-slate-500" /></div>
        {searching && <p className="mt-4 px-1 text-sm text-slate-500">Searching...</p>}
        {!searching && q.trim().length >= 2 && results && totalResults === 0 && <p className="mt-4 px-1 text-sm text-slate-500">No matches for "{q}".</p>}
        {!searching && results && results.leads.length > 0 && <div className="mt-4"><div className="mb-1.5 px-1 text-xs font-semibold uppercase tracking-wider text-slate-500">Leads</div>{results.leads.map((l) => <div key={l.id} className="mb-1.5 rounded-xl bg-white/[.03] p-3 text-sm"><div className="font-medium text-white">{l.name}</div><div className="text-xs text-slate-400"><a href={`tel:${l.phone}`} className="text-cyan-300">{l.phone}</a> • {l.status} • {l.lead_owner}</div></div>)}</div>}
        {!searching && results && results.accounts.length > 0 && <div className="mt-4"><div className="mb-1.5 px-1 text-xs font-semibold uppercase tracking-wider text-slate-500">Accounts</div>{results.accounts.map((a) => <div key={a.id} className="mb-1.5 rounded-xl bg-white/[.03] p-3 text-sm"><div className="font-medium text-white">{a.company_name}</div><div className="text-xs text-slate-400">{a.industry} • {a.location}</div></div>)}</div>}
        {!searching && results && results.contacts.length > 0 && <div className="mt-4"><div className="mb-1.5 px-1 text-xs font-semibold uppercase tracking-wider text-slate-500">Contacts</div>{results.contacts.map((c) => <div key={c.id} className="mb-1.5 rounded-xl bg-white/[.03] p-3 text-sm"><div className="font-medium text-white">{c.first_name} {c.last_name}</div><div className="text-xs text-slate-400">{c.phone && <a href={`tel:${c.phone}`} className="text-cyan-300">{c.phone}</a>}{c.email && ` • ${c.email}`}</div></div>)}</div>}
      </div>
    </div>}
  </>;
}
...
function Dashboard() {
  ...
  return <section><Header eyebrow="Analytics" title="CRM dashboard" subtitle="Live outbound velocity, deal value, maintenance revenue, and weighted forecast." /><div className={`grid gap-4 sm:grid-cols-2 ${f.ownerVisible ? 'lg:grid-cols-4' : 'lg:grid-cols-2'}`}><Metric label={f.ownerVisible ? 'Deal Value' : 'Your Deal Value'} value={money(f.rawPipeline)} bar="bg-cyan-300" /><Metric label={f.ownerVisible ? 'Monthly Maintenance' : 'Your Monthly Maintenance'} value={money(f.monthlyMaintenance)} bar="bg-emerald-300" />{f.ownerVisible && <><Metric label="Expected Revenue" value={money(f.expectedRevenue)} bar="bg-violet-300" /><Metric label="Retainer MRR" value={money(f.mrr)} bar="bg-amber-300" /></>}</div>
...
function LeadsPanel({ profile }: { profile: SalesRep }) {
  ...
  const isStale = (l: Lead) => l.status === 'New' && l.call_count === 0 && (Date.now() - new Date(l.created_at).getTime()) >= 3 * 86400000;
...
function AccountsContacts() {
  const [data, setData] = useState<{ accounts: Account[]; contacts: Contact[]; opportunities: Opportunity[]; activities: any[] } | null>(null), [loading, setLoading] = useState(true), [error, setError] = useState('');
  ...
  return <section><Header eyebrow="Database" title="Accounts & contacts" subtitle="Centralized company records linked to contacts and revenue opportunities." /><div className="grid gap-4">{data.accounts.map((a) => { const contacts = data.contacts.filter((c) => c.account_id === a.id); const opps = data.opportunities.filter((o) => o.account_id === a.id); const oppIds = opps.map((o) => o.id); const timeline = (data.activities || []).filter((act) => oppIds.includes(act.opportunity_id)); const isOpen = expandedId === a.id; return <div key={a.id} className="rounded-3xl border border-white/10 bg-[#101218] p-5"><div className="flex flex-col justify-between gap-4 md:flex-row"><div><h3 className="text-xl font-semibold">{a.company_name}</h3><p className="text-sm text-slate-400">{a.industry} • {a.location}</p></div><div className="flex items-center gap-2"><Badge>{contacts.length} contacts</Badge><Badge>{opps.length} deals</Badge><button onClick={() => setExpandedId(isOpen ? null : a.id)} className="rounded-full bg-white/5 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-white/10">{isOpen ? 'Hide Timeline' : `View Timeline (${timeline.length})`}</button></div></div><div className="mt-5 grid gap-4 lg:grid-cols-2"><div><div className="mb-2 text-xs uppercase text-slate-500">Contacts</div>{contacts.map((c) => <div key={c.id} className="mb-2 rounded-xl bg-white/[.03] p-3 text-sm"><div className="font-medium">{c.first_name} {c.last_name} <span className="text-slate-500">— {c.role_title}</span></div><div className="text-slate-400"><a href={`tel:${c.phone}`} className="text-cyan-300">{c.phone}</a> {c.email && `• ${c.email}`}</div></div>)}</div><div><div className="mb-2 text-xs uppercase text-slate-500">Opportunities</div>{opps.map((o) => <div key={o.id} className="mb-2 rounded-xl bg-white/[.03] p-3 text-sm"><div className="font-medium">{o.deal_name}</div><div className="text-slate-400">{o.stage} • {money(o.amount)} • TCV {money(tcv(o))}</div></div>)}</div></div>
    {isOpen && <div className="mt-5 border-t border-white/5 pt-4"><div className="mb-2 text-xs uppercase text-slate-500">Activity Timeline</div>{timeline.length === 0 && <p className="text-sm text-slate-500">No activity logged yet for this account.</p>}<div className="space-y-2 max-h-72 overflow-y-auto pr-1">{timeline.map((act: any) => <div key={act.id} className="rounded-xl bg-white/[.03] p-3 text-sm"><div className="flex items-center justify-between"><span className="font-medium text-white">{act.type}</span><span className="text-xs text-slate-500">{new Date(act.created_at).toLocaleDateString()} • {act.rep_email}</span></div>{act.note && <p className="mt-1 text-slate-400">{act.note}</p>}</div>)}</div></div>}
    </div>; })}</div></section>;
}
...
function AdminPortal() {
  ...
  return (
    <section>
      ...
      <div className="mt-6 rounded-3xl border border-white/10 bg-[#101218] p-5">
        <div className="mb-4 text-sm font-semibold text-slate-200">All Leads — Reassign Owner</div>
        <div className="overflow-x-auto"><table className="w-full min-w-[600px] text-left text-sm"><thead className="text-xs uppercase text-slate-500"><tr><th className="pb-2">Lead</th><th>Phone</th><th>Status</th><th>Owner</th></tr></thead><tbody>{data.leads.map((l: Lead) => <tr key={l.id} className="border-t border-white/5"><td className="py-2 font-medium text-white">{l.name}</td><td className="text-slate-400">{l.phone}</td><td><StatusPill status={l.status} /></td><td><select value={l.lead_owner} disabled={reassigning === l.id} onChange={(e) => reassignLead(l.id, e.target.value)} className="field !py-1.5 !px-2 text-xs">{data.reps.map((r: SalesRep) => <option key={r.email} value={r.email}>{r.name}</option>)}</select>{reassigning === l.id && <span className="ml-2 text-xs text-slate-500">saving...</span>}</td></tr>)}</tbody></table></div>
      </div>
